...
/**
 * Extract editable text from the ProseMirror document, separating blocks with `\n`.
 * Builds a position map `posAt[i]` = ProseMirror position of char `i`.
 * Skips code blocks.
 */
...
function getApiBase(): string {
  const env = (typeof import.meta !== 'undefined' && (import.meta as any).env?.VITE_LANGUAGETOOL_API_URL) || '';
  return (env as string).trim().replace(/\/+$/, '') || 'https://api.languagetool.org/v2';
}
